...
def sliding_window(image, neighbour_effect, stepSize, windowSize):
    ...
    distance_window = make_spatial_distance_window(windowSize,
                                                   center_coordi)  # Euclidean distance matrix between center coordination and neighbor cordination
    ...
    for y in range(0, image.shape[0] - int(windowSize[0] / 2), stepSize):
        for x in range(0, image.shape[1] - int(windowSize[1] / 2), stepSize):
            ...
            gray_deviation = np.sqrt(
                np.sum(pow(difference_nei_cent * reliable_mat, 2)) / np.count_nonzero(reliable_mat))
...
class MFCM():
    ...
    def update_U(self):
        ...
        return self.neighbour_effect * numerator / denominator[:, None]
    ...
    def get_filtered_image(self):
        ...
        pad_img = cv2.copyMakeBorder(self.image, pad_size_y, pad_size_y, pad_size_x, pad_size_x, cv2.BORDER_CONSTANT,
                                     value=0)  # zero padding

        filtered_image = sliding_window(pad_img, self.neighbour_effect, stepSize=1,
                                        windowSize=(self.kernel_size, self.kernel_size))
...
def main(DIRECTORY, args):
    ...
    try:
        ...
        # --------------Clustering--------------
        cluster = MFCM(img, image_bit=args.num_bit, n_clusters=args.n_clusters, m=args.fuzziness,
                       neighbour_effect=args.neighbour_effect, epsilon=args.epsilon, max_iter=args.max_iter,
        ...
        if IS_SAVE:
            ...
            filtered_img_path = os.path.join(OUTPUT_FILT_IMG_PATH, "%s.png" % (os.path.basename(IMG_PATH)[:-4]))
            plt.axis('off')
            plt.imshow(cluster.filtered_image)  # , cmap='gray'
            plt.savefig(filtered_img_path, bbox_inches='tight', dpi=300)
            plt.close()
